<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Espacio Praxis</title>

    <!-- Bootstrap -->
    <link href="css/bootstrap.min.css" rel="stylesheet">
    <link href="css/normalize.css" rel="stylesheet">
    <link href="css/font-awesome.min.css" rel="stylesheet">
    <link href="css/flexslider.css" rel="stylesheet">
    <link href="style.css" rel="stylesheet">

    <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->
    <script src="js/modernizr.js" type="text/javascript"></script>
	<script src="//use.typekit.net/anz0iba.js"></script>
    <script>try{Typekit.load();}catch(e){}</script>
  </head>
  <body>
  	<header id="nohome">
    	<div class="container no-column">
        	<div id="logo-head" class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
            	<a href="index.php" class="logo-grande"><img src="img/logo-rojo.png" alt=""></a>
                <a href="index.php" class="logo-chico ocultar"><img src="img/logo-chico-rojo.png" alt=""></a>
            </div>
        	<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 no-column">
                <nav> 
                  <ul> 
                    <li><a href="#">Somos</a></li> 
                    <li><a href="#">Actividad</a></li> 
                    <li><a href="publicaciones.php">Publicaciones</a></li> 
                    <li><a href="financiamiento.php">Financiamiento</a></li> 
                    <li><a href="contacto.php">Contacto</a></li> 
                  </ul> 
                </nav>
            </div>
        	<div id="buscador" class="col-lg-2 col-md-2 col-sm-2 col-xs-12 text-center no-column">
            	<form>
                    <input type="text">
                    <button type="submit">
                    	<i class="fa fa-search fa-2x"></i>
                    </button>
                </form>
            </div>
            
            <div class="icons-rs">
            	<i class="fa fa-facebook"></i>
                <i class="fa fa-twitter"></i>
            </div>
            
        </div>
    </header>
    
    
    <div id="contenido" class="box fwidth fleft">
    	<!--loop--->
        
        <section id="imagen-titular" class="box fwidth fleft" style="background:url(img/header-financiamiento.jpg) center center no-repeat;-webkit-background-size: cover;-moz-background-size: cover;-o-background-size: cover;background-size: cover;">
        </section>

        <div class="container">
            <div id="financiamiento" class="col-lg-8 col-md-8 col-sm-8 col-xs-10 col-lg-offset-2 col-md-offset-2 col-sm-offset-2 col-xs-offset-1 text-left">
            	
                <h2>Financiamiento</h2>
                <p><strong>PRAXIS</strong> requiere, para cumplir con sus objetivos, contar con un financiamiento regular y sostenido en el tiempo. Uno de sus principios es la autonomía respecto a los poderes políticos y económicos y, para ello, es importante no depender de su dinero. Por tanto, su forma de financiarse es a través de aportaciones solidarias y desinteresadas de quienes comparten sus objetivos. Te invitamos a ser parte de quienes ayudan a que un proyecto como PRAXIS sea posible.
                </p>
                <a href="#">aportar</a>
				
                
            </div>
        </div>
        
        
        
        <!--/loop-->
    </div>
    <!--/contenido-->

	<footer class="container-fluid no-column">
    	<div class="container no-column">
        	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-left no-column">
            	
                <div class="box fwidth fleft">
                	<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2 col-lg-offset-1 col-md-offset-1 col-sm-offset-1 col-xs-offset-1">
                    	<p>&copy; 2015</p>
                        <p>Espacio Praxis</p>                        
                    </div>
                	<div class="col-lg-6 col-md-6 col-sm-4 col-xs-4 col-lg-offset-1 col-md-offset-1 col-sm-offset-1 col-xs-offset-1">
                    	<p>Eulogia Sanchez 065</p>
                        <p>Santiago de Chile</p>                        
                    </div>
                	<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
                    	<p>Desarrollado</p>
                        <p>por <a href="#">Cerro</a></p>                        
                    </div>
                </div>
                
            </div>
        </div>

    </footer>
    

    <!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
    <!-- Include all compiled plugins (below), or include individual files as needed -->
    <script src="js/bootstrap.min.js"></script>
    <script src="js/scripts.js"></script>
    <script src="js/jquery.flexslider-min.js"></script>
  </body>
</html>